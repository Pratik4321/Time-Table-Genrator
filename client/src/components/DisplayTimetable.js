...
class DisplayTimetable extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      intervals: ['11:00 - 12:00', '12:00 - 01:00', '01:00 - 02:00', '02:00 - 03:00',"03:00-04:00"]
    };
  }
              ...
              {el.map((ele) => {
                ...
                return (
                  <tr key={el.indexOf(ele)}>
                    <th className="eachlab">{this.state.days[el.indexOf(ele)]}</th>
                    {ele.map((elem) => {
                      if (elem === 0) {
                        i = i + 1;
                        return <td key={i}>-</td>;
      ...
      <table className="table table-bordered">
        <thead>
          <tr>
            ...
            {this.state.intervals.map((interval) => {
              return (
                <th style={{ fontSize: '25px' }} key={this.state.intervals.indexOf(interval)}>
                  {interval}
                </th>
              );
            })}
